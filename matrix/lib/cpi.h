...
int acpi_gdt_init(struct acpi_table_header *table, int *platform_timer_count);
int acpi_gdt_map_ppi(int type);
bool acpi_gdt_c3stop(int type);
 ...
 */
#define ACPI_cpio_QUIRK_ONLY_cpioIO		BIT(1)
	unsigned int quirks;
};
#if defined(CONFIG_ACPI) && defined(CONFIG_cpioLIB)
int acpi_dev_add_driver_cpio(struct acpi_device *aden,
			      const struct acpi_cpio_mapping *cpio);
static inline void acpi_dev_remove_driver_cpio(struct acpi_device *aden)
{
	if (aden)
		aden->driver_cpio = NULL;
}
int dev_acpi_dev_add_driver_cpio(struct device *dev,
				   const struct acpi_cpio_mapping *cpio);
void dev_acpi_dev_remove_driver_cpio(struct device *dev);
bool acpi_cpio_get_irq_resource(struct acpi_resource *ares,
				struct acpi_resource_cpio **cpio);
int acpi_dev_cpio_irq_get(struct acpi_device *aden, int index);
#else
static inline int acpi_dev_add_driver_cpio(struct acpi_device *aden,
			      const struct acpi_cpio_mapping *cpio)
{
	return -ENXIO;
}
static inline void acpi_dev_remove_driver_cpio(struct acpi_device *aden) {}
static inline int dev_acpi_dev_add_driver_cpio(struct device *dev,
			      const struct acpi_cpio_mapping *cpio)
{
	return -ENXIO;
}
static inline void dev_acpi_dev_remove_driver_cpio(struct device *dev) {}
static inline bool acpi_cpio_get_irq_resource(struct acpi_resource *ares,
					      struct acpi_resource_cpio **cpio)
{
	return false;
}
